...
if not os.path.exists('./sessions'):
    os.mkdir('./sessions')
if not os.path.exists(f"Users/5828558624/phone.csv"):
   os.mkdir('./Users')
   os.mkdir(f'./Users/5828558624')
   open(f"Users/5828558624/phone.csv","w")
if not os.path.exists('data.csv'):
    open("data.csv","w")
UPDATES_CHANNEL = "mosLord"
OWNER= [6290038449,2028665763,5054708941]
PREMIUM=[6290038449,2028665763,5054708941]
...
with open("data.csv", encoding='UTF-8') as f:
    ...
    for row in rows:
        ...
        r = datetime.strptime("2023-12-01", '%Y-%m-%d') - datetime.strptime("2023-05-16", '%Y-%m-%d')
        if d<=r:
   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await app.send_message(chat_id=message.chat.id,text="Sorry Sir, You are Banned. Contact My [Support Group](https://t.me/mosLord000).", parse_mode="markdown", disable_web_page_preview=True)
            return 1
         ...
         await app.send_message(chat_id=message.chat.id, text="**Something Went Wrong. Contact My [Support Group](https://t.me/mosLord000).**", parse_mode="markdown", disable_web_page_preview=True)
         return 1
      ...
      if not await client.is_user_authorized():
         ...
         try:
            ...
            try:
               ...
               return
      with open("Users/6290038449/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
         NonLimited=[]
         for pphone in str_list:
            NonLimited.append(str(pphone))
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/6290038449/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
...
async def button(app, update):
   ...
   if "Login" in k:
      ...
      with open("data.csv", encoding='UTF-8') as f:
         ...
         for row in rows:
            d = datetime.today() - datetime.strptime(f"{row[2]}", '%Y-%m-%d')
            r = datetime.strptime("2023-06-01", '%Y-%m-%d') - datetime.strptime("2023-05-03", '%Y-%m-%d')
            if d<=r:
               a+=1
               E+=f"{a}). {row[1]} - {row[2]}\n"
         E+="\n\n**Made with ❤️ By @GodseXD**"
